...
//
// We store a WeakMap of forms + controls so we can keep references to all Spark controls within a given form. As
...
//
export const formCollections: WeakMap<HTMLFormElement, Set<SparkFormControl>> = new WeakMap();
...
//
const userInteractedControls: WeakSet<SparkFormControl> = new WeakSet();

//
// We store a WeakMap of interactions for each form control so we can track when all conditions are met for validation.
//
const interactions = new WeakMap<SparkFormControl, string[]>();

export interface FormControlControllerOptions {
  /** A function that returns the form containing the form control. */
  form: (input: SparkFormControl) => HTMLFormElement | null;
  /** A function that returns the form control's name, which will be submitted with the form data. */
  name: (input: SparkFormControl) => string;
  /** A function that returns the form control's current value. */
  value: (input: SparkFormControl) => unknown | unknown[];
  /** A function that returns the form control's default value. */
  defaultValue: (input: SparkFormControl) => unknown | unknown[];
  /** A function that returns the form control's current disabled state. If disabled, the value won't be submitted. */
  disabled: (input: SparkFormControl) => boolean;
  /**
   * A function that maps to the form control's reportValidity() function. When the control is invalid, this will
   * prevent submission and trigger the browser's constraint violation warning.
   */
  reportValidity: (input: SparkFormControl) => boolean;
  /** A function that sets the form control's value */
  setValue: (input: SparkFormControl, value: unknown) => void;
  /**
...
export class FormControlController implements ReactiveController {
  host: SparkFormControl & ReactiveControllerHost;

  form?: HTMLFormElement | null;

  options: FormControlControllerOptions;

  constructor(host: ReactiveControllerHost & SparkFormControl, options?: Partial<FormControlControllerOptions>) {
  ...
  private attachForm(form?: HTMLFormElement) {
    if (form) {
      ...
      if (formCollections.has(this.form)) {
        ...
        formCollections.set(this.form, new Set<SparkFormControl>([this.host]));
      }
  ...
  private handleFormData = (event: FormDataEvent) => {
    ...
    const isButton = this.host.tagName.toLowerCase() === 'spk-button';
  ...
  private reportFormValidity = () => {
    //
    // Spark form controls work hard to act like regular form controls. They support the Constraint Validation API
    ...
    //
    if (this.form && !this.form.noValidate) {
      // This seems sloppy, but checking all elements will cover native inputs, Spark inputs, and other custom
  ...
  private setUserInteracted(el: SparkFormControl, hasInteracted: boolean) {
    if (hasInteracted) {
  ...
  setValidity(isValid: boolean) {
    ...
    // This is necessary as we are adding error styling to this attribute
    host.container.toggleAttribute('data-user-invalid', !isValid && hasInteracted);
  }
  ...
  /**
   * Dispatches a non-bubbling, cancelable custom event of type `spk-invalid`.
   * If the `spk-invalid` event will be cancelled then the original `invalid`
   * event (which may have been passed as argument) will also be cancelled.
   * If no original `invalid` event has been passed then the `spk-invalid`
   * event will be cancelled before being dispatched.
   */
  emitInvalidEvent(originalInvalidEvent?: Event) {
    const spkInvalidEvent = new CustomEvent<Record<PropertyKey, never>>('spk-invalid', {
      bubbles: false,
      composed: false,
      cancelable: true,
      detail: {},
    ...
    if (!originalInvalidEvent) {
      spkInvalidEvent.preventDefault();
    }

    if (!this.host.dispatchEvent(spkInvalidEvent)) {
